...
class AssessmentService {
  ...
  static Future<List<Assessment>> getModuleAssessments({
    ...
    print('🎯 AssessmentService: Fetching assessments for module $moduleId in course $courseId');
    ...
    try {
      ...
      print('✅ Found ${assessments.length} assessments for module $moduleId');
      for (final assessment in assessments) {
        print('   ├─ ${assessment.title} (${assessment.totalQuestions} questions)');
      }
      
      return assessments;
    } catch (e) {
      print('❌ Error fetching module assessments: $e');
      return [];
    }
  }
  ...
  static Future<List<Question>> getAssessmentQuestions({
    ...
    print('❓ AssessmentService: Fetching questions for assessment $assessmentId');
    ...
    try {
      ...
      print('✅ Found ${questions.length} questions for assessment $assessmentId');
      for (int i = 0; i < questions.length; i++) {
        final q = questions[i];
        print('   ├─ Q${i + 1}: ${q.question.substring(0, q.question.length.clamp(0, 50))}...');
      }
      
      return questions;
    } catch (e) {
      print('❌ Error fetching assessment questions: $e');
      return [];
    }
  }
}